...
cultureUI <- function(id) {
  ...
  shiny::tagList(
    shiny::tags$style("
      ...
      .cul-export-row { display:grid; grid-template-columns:repeat(3,minmax(220px,1fr)); gap:10px; }
    ...
    shiny::div(
      ...
      shiny::div(
        ...
        shiny::div(
          class = "cul-export",
          shiny::div(
            shiny::div(class = "cul-card-title", "Specimen CSV"),
            shiny::div(class = "cul-export-path", shiny::textOutput(ns("specimen_export_path"), inline = TRUE))
          ),
          shiny::actionButton(ns("export_specimen_csv"), "Export specimen CSV", class = "btn btn-outline-primary btn-sm")
        )
      ),
      shiny::div(
        class = "cul-grid",
...
cultureServer <- function(id, app_state) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    last_specimen_export <- shiny::reactiveVal("Not exported yet")
    ...
    output$specimen_export_path <- shiny::renderText(last_specimen_export())
    ...
    shiny::observeEvent(input$export_specimen_csv, {
      tryCatch({
        info <- write_cfu_specimen_csv(
          filtered_data(),
          batch_id = culture_batch_id(),
          output_dir = file.path("data", "exports")
        )
        last_specimen_export(info$path)
        shiny::showNotification(
          sprintf("Wrote %s specimen-level rows to %s.", format(info$n, big.mark = ","), info$path),
          type = "message",
          duration = 6
        )
      }, error = function(e) {
        shiny::showNotification(paste("CFU specimen CSV export failed:", e$message),
                                type = "error", duration = 10)
        warning("AXIS export_specimen_csv failed: ", e$message)
      })
    })
